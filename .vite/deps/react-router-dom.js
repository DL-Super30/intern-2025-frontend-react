...
// node_ModulesPage/cookie/dist/index.js
var require_dist = __commonJS({
  "node_ModulesPage/cookie/dist/index.js"(exports) {
    "use strict";
...
// node_ModulesPage/set-cookie-parser/lib/set-cookie.js
var require_set_cookie = __commonJS({
  "node_ModulesPage/set-cookie-parser/lib/set-cookie.js"(exports, module) {
    "use strict";
...
// node_ModulesPage/react-router/dist/development/chunk-AYJ5UCUI.mjs
...
// node_ModulesPage/turbo-stream/dist/turbo-stream.mjs
var HOLE = -1;
var NAN = -2;
...
async function loadRouteModule(route, routeModulesPageCache) {
  if (route.id in routeModulesPageCache) {
    return routeModulesPageCache[route.id];
  }
  try {
    ...
    routeModulesPageCache[route.id] = routeModule;
    return routeModule;
  } catch (error) {
    console.error(
...
function getKeyedLinksForMatches(matches, routeModulesPage, manifest) {
  let descriptors = matches.map((match) => {
    var _a;
    let module = routeModulesPage[match.route.id];
...
async function getKeyedPrefetchLinks(matches, manifest, routeModulesPage) {
  let links = await Promise.all(
    matches.map(async (match) => {
      let route = manifest.routes[match.route.id];
      if (route) {
        let mod = await loadRouteModule(route, routeModulesPage);
...
function getTurboStreamSingleFetchDataStrategy(getRouter, manifest, routeModulesPage, ssr, basename) {
  let dataStrategy = getSingleFetchDataStrategyImpl(
    getRouter,
    (match) => {
      let manifestRoute = manifest.routes[match.route.id];
      invariant2(manifestRoute, "Route not found in manifest");
      let routeModule = routeModulesPage[match.route.id];
      return {
...
function BoundaryShell({
  ...
  var _a;
  let { routeModulesPage } = useFrameworkContext();
  if (((_a = routeModulesPage.root) == null ? void 0 : _a.Layout) && !isOutsideRemixApp) {
    return children;
  }
      ...
      dangerouslySetInnerHTML: {
        __html: `
              console.log(
                ...
                "when your app is loading JS ModulesPage and/or running \`clientLoader\` " +
...
function createServerRoutes(manifest, routeModulesPage, future, isSpaMode, parentId = "", routesByParentId = groupRoutesByParentId(manifest), spaModeLazyPromise = Promise.resolve({ Component: () => null })) {
  return (routesByParentId[parentId] || []).map((route) => {
    let routeModule = routeModulesPage[route.id];
    invariant2(
      routeModule,
...
function createClientRoutesWithHMRRevalidationOptOut(needsRevalidation, manifest, routeModulesPageCache, initialState, ssr, isSpaMode) {
  return createClientRoutes(
    manifest,
    routeModulesPageCache,
    initialState,
    ssr,
    isSpaMode,
    "",
...
function createClientRoutes(manifest, routeModulesPageCache, initialState, ssr, isSpaMode, parentId = "", routesByParentId = groupRoutesByParentId(manifest), needsRevalidation) {
  return (routesByParentId[parentId] || []).map((route) => {
    var _a, _b, _c;
    let routeModule = routeModulesPageCache[route.id];
    ...
    async function prefetchStylesAndCallHandler(handler) {
      let cachedModule = routeModulesPageCache[route.id];
...
async function loadRouteModuleWithBlockingLinks(route, routeModulesPage) {
  let routeModulePromise = loadRouteModule(route, routeModulesPage);
...
function getPatchRoutesOnNavigationFunction(manifest, routeModulesPage, ssr, isSpaMode, basename) {
...
function useFogOFWarDiscovery(router2, manifest, routeModulesPage, ssr, isSpaMode) {
  React8.useEffect(() => {
    var _a;
  ...
  }, [ssr, isSpaMode, manifest, routeModulesPage, router2]);
}
var MANIFEST_VERSION_STORAGE_KEY = "react-router-manifest-version";
async function fetchAndApplyManifestPatches(paths, errorReloadPath, manifest, routeModulesPage, ssr, isSpaMode, basename, patchRoutes, signal) {
  ...
  parentIds.forEach(
    (parentId) => patchRoutes(
      parentId || null,
      createClientRoutes(patches, routeModulesPage, null, ssr, isSpaMode, parentId)
    )
  );
}
...
function Links() {
  let { isSpaMode, manifest, routeModulesPage, criticalCss } = useFrameworkContext();
  let { errors, matches: routerMatches } = useDataRouterStateContext();
  let matches = getActiveMatches(routerMatches, errors, isSpaMode);
  let keyedLinks = React9.useMemo(
    () => getKeyedLinksForMatches(matches, routeModulesPage, manifest),
    [matches, routeModulesPage, manifest]
  );
...
function useKeyedPrefetchLinks(matches) {
  let { manifest, routeModulesPage } = useFrameworkContext();
  let [keyedPrefetchLinks, setKeyedPrefetchLinks] = React9.useState([]);
  React9.useEffect(() => {
    let interrupted = false;
    void getKeyedPrefetchLinks(matches, manifest, routeModulesPage).then(
      (links) => {
        if (!interrupted) {
  ...
  }, [matches, manifest, routeModulesPage]);
...
function PrefetchPageLinksImpl({
  ...
  let dataHrefs = React9.useMemo(() => {
    ...
    nextMatches.forEach((m) => {
      ...
      if (!newMatchesForData.some((m2) => m2.route.id === m.route.id) && m.route.id in loaderData && ((_a = routeModulesPage[m.route.id]) == null ? void 0 : _a.shouldRevalidate)) {
        foundOptOutRoute = true;
...
function Meta() {
  let { isSpaMode, routeModulesPage } = useFrameworkContext();
  let {
    errors,
  ...
  for (let i = 0; i < _matches.length; i++) {
    ...
    let routeModule = routeModulesPage[routeId];
    let routeMeta = [];
    let match = {
      id: routeId,
      data: data2,
...
function Scripts(props) {
  ...
  let initialScripts = React9.useMemo(() => {
    ...
    let routeModulesPageScript = !isStatic ? " " : `${((_a = manifest.hmr) == null ? void 0 : _a.runtime) ? `import ${JSON.stringify(manifest.hmr.runtime)};` : ""}${!enableFogOfWar ? `import ${JSON.stringify(manifest.url)}` : ""};
${matches.map((match, routeIndex) => {
  ...
  window.__reactRouterRouteModulesPage = {${matches.map((match, index) => `${JSON.stringify(match.route.id)}:route${index}`).join(",")}};
...
import(${JSON.stringify(manifest.entry.module)});`;
    return React9.createElement(React9.Fragment, null, React9.createElement(
      ...
      "script",
      {
        ...props,
        suppressHydrationWarning: true,
        dangerouslySetInnerHTML: createHtml(routeModulesPageScript),
        type: "module",
        async: true
      }
    ));
...
function ServerRouter({
  ...
  let { manifest, routeModulesPage, criticalCss, serverHandoffString } = context;
  ...
  for (let match of context.staticHandlerContext.matches) {
    ...
    let route = routeModulesPage[routeId];
  ...
  return function RoutesTestStub({
    ...
    if (routerRef.current == null) {
      ...
      let patched = processRoutes(
        ...
        remixContextRef.current.routeModulesPage
      );
...
function processRoutes(routes, manifest, routeModulesPage, parentId) {
...
function createEntryRouteModulesPage(manifest) {
...
async function handleDocumentRequest(serverMode, build, staticHandler, request, loadContext, handleError, isSpaMode, criticalCss) {
  ...
  async function renderHtml(context, isSpaMode2) {
    ...
    let entryContext = {
      ...
      routeModulesPage: createEntryRouteModulesPage(build.routes),
...
// node_ModulesPage/react-router/dist/development/dom-export.mjs
...
function initSsrInfo() {
  if (!ssrInfo && window.__reactRouterContext && window.__reactRouterManifest && window.__reactRouterRouteModulesPage) {
    ...
    ssrInfo = {
      ...
      routeModulesPage: window.__reactRouterRouteModulesPage,
  ...
  if (ssrInfo.context.isSpaMode) {
    ...
    hydrationData = getHydrationData(
      ...
      (routeId) => {
        var _a2, _b2, _c;
        return {
          clientLoader: (_a2 = ssrInfo.routeModulesPage[routeId]) == null ? void 0 : _a2.clientLoader,
          hasLoader: ((_b2 = ssrInfo.manifest.routes[routeId]) == null ? void 0 : _b2.hasLoader) === true,
          hasHydrateFallback: ((_c = ssrInfo.routeModulesPage[routeId]) == null ? void 0 : _c.HydrateFallback) != null
        };
      },
      window.location,
    ...
    React22.createElement(React22.Fragment, null, React22.createElement(
      FrameworkContext.Provider,
      {
        value: {
          ...
          routeModulesPage: ssrInfo.routeModulesPage,
