...
const ModulesPage = ({ selectedCourse }) => {
  ...
  const handleAddModule = () => {
    setModulesPage([
      ...ModulesPage,
      {
        title: `New Module ${ModulesPage.length + 1}`,
        videos: [{ name: '', link: '' }],
      },
    ]);
  };
  ...
  const handleAddVideo = (moduleIndex) => {
    const newModulesPage = [...ModulesPage];
    newModulesPage[moduleIndex].videos.push({ name: '', link: '' });
    setModulesPage(newModulesPage);
  };

  const handleDeleteVideo = (moduleIndex, videoIndex) => {
    const newModulesPage = [...ModulesPage];
    newModulesPage[moduleIndex].videos.splice(videoIndex, 1);
    setModulesPage(newModulesPage);
  };

  const handleDeleteModule = (moduleIndex) => {
    const newModulesPage = [...ModulesPage];
    newModulesPage.splice(moduleIndex, 1);
    setModulesPage(newModulesPage);
  };

  const handleInputChange = (moduleIndex, videoIndex, field, value) => {
    const newModulesPage = [...ModulesPage];
    newModulesPage[moduleIndex].videos[videoIndex][field] = value;
    setModulesPage(newModulesPage);
  };

  const handleTitleChange = (moduleIndex, value) => {
    const newModulesPage = [...ModulesPage];
    newModulesPage[moduleIndex].title = value;
    setModulesPage(newModulesPage);
  };
  ...
  return (
    <div className="p-6">
      ...
      {ModulesPage.map((module, moduleIndex) => (
        <div
          key={moduleIndex}
